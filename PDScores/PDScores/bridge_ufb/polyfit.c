      ...
      if (i + 1 < 3) {
        ...
        if (lastv + 1 > 0) {
          if (lastc == 0) {
            ...
            iy = 0;
            i24 = i_ip1 + b_m * (lastc - 1);
            itemp = i_ip1;
            while ((b_m > 0) && (itemp <= i24)) {
              ix = i_i;
              smax = 0.0;
              pvt = itemp + lastv;
              for (k = itemp; k <= pvt; k++) {
...
void b_polyfit(const emxArray_real_T *x, const emxArray_real_T *y, double p[3])
{
  ...
  V->size[1] = 3;
  emxEnsureCapacity((emxArray__common *)V, i23, (int)sizeof(double));
...
void polyfit(const emxArray_real_T *x, const emxArray_real_T *y, double p[2])
{
  ...
  V->size[0] = (int)unnamed_idx_0;
  V->size[1] = 2;
  emxEnsureCapacity((emxArray__common *)V, i7, (int)sizeof(double));
  if ((int)unnamed_idx_0 == 0) {
  } else {
    for (k = 0; k <= n; k++) {
