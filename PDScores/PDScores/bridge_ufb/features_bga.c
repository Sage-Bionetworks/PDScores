...
void features_bga(const emxArray_real_T *gait, double ft[7])
{
  ...
  if (gait->size[0] == 0) {
    ...
    for (npages = 0; npages < n; npages++) {
      t->data[npages] = gait->data[npages] - dT;
    }
    ...
    if (xlast < 19.0) {
      ...
      for (ii_size_idx_0 = 0; ii_size_idx_0 < n; ii_size_idx_0++) {
        b_gaitmag->data[ii_size_idx_0] = ((gaitmag->data[ii_size_idx_0] < dT) &&
          (gaitmag->data[npages + ii_size_idx_0] >= dT));
      }
      ...
      n = gaitmag->size[0];
      for (npages = 0; npages < n; npages++) {
        c_gaitmag->data[npages] = gaitmag->data[npages] - dT;
      }
      ...
      if (gaitmag->size[0] > 1) {
        ...
        if (ixstart < gaitmag->size[0]) {
          while (ixstart + 1 <= n) {
            if (gaitmag->data[ixstart] > xlast) {
              xlast = gaitmag->data[ixstart];
              idx = ixstart;
            }
