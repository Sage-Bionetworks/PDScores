/* Performs fast detrended fluctuation analysis on a nonstationary input signal.
   ...
   If you use this code in academic publications, please cite:
...
//#endif
//)
void fastdfa_core_nomex(double *intvs_out, double *fluct_out, double *x_in, unsigned long rowsIn, unsigned long colsIn)
{
   unsigned long  samples, dimensions, elements;
   ...
   /* Calculate accumulated sum signal */
   y_in = (REAL *)calloc(elements, sizeof(REAL));
...
//   else
   {
      ...
      intervals = (unsigned long *)calloc(N_scales, sizeof(unsigned long));
      calculateIntervals(elements, N_scales, intervals);
   }

   /* Calculate number of scales needed */
   flucts = (REAL *)calloc(N_scales, sizeof(REAL));
